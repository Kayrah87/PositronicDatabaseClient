<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        // Create users table for authentication
        if (!Schema::hasTable('users')) {
            Schema::create('users', function (Blueprint $table) {
                $table->id();
                $table->string('name');
                $table->string('email')->unique();
                $table->timestamp('email_verified_at')->nullable();
                $table->string('password');
                $table->rememberToken();
                $table->timestamps();
            });
        }

        // Create password reset tokens table
        if (!Schema::hasTable('password_reset_tokens')) {
            Schema::create('password_reset_tokens', function (Blueprint $table) {
                $table->string('email')->primary();
                $table->string('token');
                $table->timestamp('created_at')->nullable();
            });
        }

        // Create sessions table for database session storage
        if (!Schema::hasTable('sessions')) {
            Schema::create('sessions', function (Blueprint $table) {
                $table->string('id')->primary();
                $table->foreignId('user_id')->nullable()->index();
                $table->string('ip_address', 45)->nullable();
                $table->text('user_agent')->nullable();
                $table->longText('payload');
                $table->integer('last_activity')->index();
            });
        }

        // Create database connections table for storing connection configs
        if (!Schema::hasTable('database_connections')) {
            Schema::create('database_connections', function (Blueprint $table) {
                $table->id();
                $table->string('name');
                $table->string('driver'); // mysql, pgsql, sqlite, sqlsrv, mongodb
                $table->string('host')->nullable();
                $table->integer('port')->nullable();
                $table->string('database');
                $table->string('username')->nullable();
                $table->text('password')->nullable(); // encrypted
                $table->json('options')->nullable(); // additional connection options
                $table->boolean('is_active')->default(true);
                $table->timestamps();
            });
        }

        // Create query history table for storing executed queries
        if (!Schema::hasTable('query_history')) {
            Schema::create('query_history', function (Blueprint $table) {
                $table->id();
                $table->foreignId('user_id')->nullable()->constrained()->onDelete('cascade');
                $table->foreignId('connection_id')->constrained('database_connections')->onDelete('cascade');
                $table->text('query');
                $table->integer('execution_time')->nullable(); // milliseconds
                $table->integer('rows_affected')->nullable();
                $table->text('result')->nullable(); // JSON result for SELECT queries
                $table->text('error')->nullable(); // Error message if query failed
                $table->timestamps();
            });
        }
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('query_history');
        Schema::dropIfExists('database_connections');
        Schema::dropIfExists('sessions');
        Schema::dropIfExists('password_reset_tokens');
        Schema::dropIfExists('users');
    }
};